...
A FLAC encoder is free in selecting which method is used to model the input, however, a constant subframe can only be used when the signal is constant and linear or fixed predictor subframes can only be used when their all their residual samples are representable in 32-bit signed integers as stated in [section coded residual](#coded-residual).
