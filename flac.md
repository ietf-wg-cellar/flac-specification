...
FLAC specifies a minimum block size of 16 and a maximum block size of 65535. This does not apply to the block size of the last frame in a stream, because the block size of that frame must be able to accomodate for the length of the audio data the file contains. As the last block is excluded from the block sizes stated in the streaminfo block, the bit patterns corresponding to the numbers 0-15 in the minimum block size and maximum block size fields are invalid.
