...
In case an escape code was used, the partition does not contain a variable-length rice coded residual, but a fixed-length unencoded residual, with each residual sample signed two's complement. Directly following the escape code are 5 bits containing the number of bits with which each residual sample is stored, as an unsigned number.
